...
updater = Updater(token='1181681703:AAEWnu1M2-XfmP0EcCL84vEQXO3oGoqQpqs', use_context=True)
...
def weatherInfo(update, context):
    ...
    response = req.get("https://api.openweathermap.org/data/2.5/weather?q=gujarat&appid=37643fffe08d4b8b453d5781ef356c29")
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text="Temperature of Gujarat is: "+temperature+"\nHumidity in Gujarat is: "+ humidity)
    # context.bot.send_message(chat_id=update.effective_chat.id, text="Humidity of gujarat is %s"%humidity)
...
def unknownCommand(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="Sorry, I didn't recognise the command.")
